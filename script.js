...
function showRandomPassage(){
    ...
    currentPassage = paragraphs[randomIndex];
    currentPassage.split("").forEach(char => {
        var span = `<span>${char}</span>`;
        passage.innerHTML += span;
    })
    spans = document.querySelectorAll('#passage span');
    spans[0].classList.add('active');
}


var mistakeCount=0;
function checkPassage(userPassage){
    ...
    if (userPassage.length < currentPassage.length){
    spans[userPassage.length].classList.add('active');
    }
    for (var i=0; i<userPassage.length; i++){
        if (userPassage.charAt(i) == currentPassage.charAt(i)){
    ...
    mistakes.innerText = "Mistakes:" + mistakeCount;

    let wpm = Math.round(((userPassage.length - mistakeCount)/5)/(maxTime-timeLeft)*60);
    wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm;
    wpmTag.innerText = "WPM: " + wpm;

    cpmTag.innerText = `CPM: ${userPassage.length - mistakeCount}`;
...
function resetGame(){
    ...
    mistakes.innerText = "Mistake: 0";
    cpmTag.innerText = "CPM: 0";
    inputbox.focus();
    started = false;
}
...
inputbox.addEventListener('input', function(){
    userPassage = inputbox.value;
    checkPassage(userPassage);
})
